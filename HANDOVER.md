...
**Rédigé le :** 29 août 2026. **Mis à jour le :** 31 août 2026 (chantier collaboration par dossier).
**Pour :** toute personne (ou IA) qui rejoint le projet sans avoir participé aux échanges précédents.
**But :** donner une vision complète — comment installer et lancer le projet depuis zéro, architecture, décisions et leur pourquoi, état actuel de chaque service externe, limites connues — sans avoir à reconstituer l'historique à partir de zéro.
...
Principe central : l'utilisateur ne désigne **jamais** ses dossiers par leur vrai nom — il choisit lui-même un nom de code pour chacun. L'app suit les échéances (audiences, rendez-vous, dates limites), relance jusqu'à confirmation que c'est fait, et propose un bilan périodique. Depuis fin août 2026, un dossier peut aussi être **partagé** avec d'autres comptes Vigie (voir section 4bis).

**Éditeur :** PENTALIGHTCODE (Tobie Vincent Amadou), Cotonou, Bénin.
**Utilisateurs réels (chiffre du 29/08/2026, à reconfirmer avec Tobie) :** 3 comptes existent, dont Tobie et son père. Pas encore publiée sur le Play Store — en cours de vérification Google (voir section 7).
...
- **Backend :** Firebase — projet `secretaire-aje`.
  ...
  - **Secret Manager** : tous les secrets serveur (voir section 9).
...
## 3. Mise en route depuis zéro — installer et lancer le projet

Cette section n'existait pas dans la version du 29/08/2026 du document — ajoutée le 31/08/2026 sur demande explicite de Tobie ("est-ce que ça leur explique minutieusement les différentes étapes... pour continuer le projet comme s'ils avaient commencé avec moi depuis le début").
...
**Étapes :**
1. Cloner le dépôt : `git clone https://github.com/pentalightcode/vigie.git` (ou l'URL SSH si un accès en écriture a été provisionné, voir section 10).
2. `google-services.json` (`app/android/app/`) et `firebase_options.dart` (`app/lib/`) sont **déjà dans le dépôt** — ce ne sont pas des secrets (identifiants client publics, la vraie protection vient des règles Firestore, voir section 9), rien à régénérer via `flutterfire configure`.
3. `cd app && flutter pub get`.
4. `firebase login` avec un compte Google ayant accès au projet `secretaire-aje` (à faire provisionner par Tobie, section 10), puis depuis `app/` : `firebase use secretaire-aje`.
...
- Les vraies valeurs des secrets serveur (Secret Manager, section 9) — inutiles sauf si vous devez modifier une fonction qui les utilise ET la tester en conditions réelles connectées à Google/Groq.
- `app/android/upload-keystore.jks` + `app/android/key.properties` (tous deux gitignorés) — nécessaires **seulement** pour builder un APK signé destiné à une vraie mise à jour publiée. Une sauvegarde existe (voir section 8), à transmettre par Tobie en main propre ou via un canal chiffré.
...
- **`PROGRESS.md` mis à jour après chaque étape significative**, jamais après coup en bloc — règle de checkpoint permanente, voir `/home/tobie/CLAUDE.md` (config globale, s'applique à tous les projets de Tobie).
- **`Notes/` pour les fiches de décision individuelles et les fiches de red team**, une par sujet — souvent avec plusieurs tours de "Red Team" successifs (challenger sa propre proposition avant de la valider, y compris en se re-relisant soi-même après coup — méthode explicitement demandée par Tobie dès la conception, voir `/home/tobie/.claude/projects/.../memory/` : "Claude fait les Red Team, Tobie décide").
...
- **Avant tout déploiement de règles/fonctions touchant à un modèle de données**, faire une vraie passe de red team (souvent plusieurs tours successifs, y compris sur ses propres correctifs — voir section 4bis pour un exemple concret) plutôt que de considérer un premier jet comme fini.

## 4bis. Collaboration par dossier — modèle "participants" (nouveau, 29-31/08/2026)
...
## 5. Principes de sécurité et de confidentialité — ce qui est réellement construit aujourd'hui
...
- Règles Firestore relues intégralement, plusieurs fois, y compris après coup sur leurs propres correctifs (voir section 4bis) : chaque collection sensible ancrée sur `uid`/`participantsUids`, champs immuables protégés à la mise à jour, forgerie de documents fermée à la création.
...
## 6. Rituel de build + déploiement (à suivre précisément)

1. Bump de version dans `app/pubspec.yaml` (`version: X.Y.Z+N`, le `+N` doit toujours augmenter).
2. `flutter analyze` — doit rester propre (2 infos pré-existantes sans rapport, voir section 3).
...
9. `firebase deploy --only hosting --force` (depuis `app/`) — **attention** : si un fichier statique dans `app/web/` a changé sans avoir refait `flutter build web` derrière, ce déploiement republie l'ancien `build/web/` sans le changement — toujours vérifier après coup avec `curl`.
...
## 7. État actuel des systèmes externes (au 31/08/2026)
...
## 8. Failles de sécurité trouvées et corrigées
...
## 9. Secrets — inventaire des emplacements (jamais les valeurs)

| Secret | Où il vit | Jamais où |
|---|---|---|
| Clé de signature Android + mot de passe | `app/android/upload-keystore.jks` + `app/android/key.properties` (tous deux gitignorés) | Sauvegarde existe mais reste sur cette machine (voir section 7) — à déplacer en priorité |
...
## 10. Accès à provisionner pour un nouveau collaborateur (à faire par Tobie personnellement, pas par ce document)
...
## 11. Ce qui reste ouvert / en attente au 31/08/2026

- Réponse de Google sur la 3ᵉ demande de vérification OAuth (voir section 7) — pas revérifié depuis le 29/08.
- Déplacer la sauvegarde du keystore Android hors de cette machine (voir section 7) — la sauvegarde existe, mais reste au même endroit que l'original.
- Carte bancaire physique pour le forfait Blaze — pas urgent.
- **Build + déploiement groupé du chantier collaboration** (règles, index, fonction Cloud `gerer_participant_dossier`, écran participants) — tout est écrit et vérifié statiquement, rien n'est encore parti en production. Voir section 4bis pour l'incertitude technique à tester en priorité juste après ce déploiement.
- Concevoir en détail le workflow d'approbation et les groupes multi-dossiers (voir section 4bis) — décisions de principe prises, rien construit.
- Clarifier la question légale APDP (Bénin) soulevée dès la conception, jamais formellement refermée — voir section 5.
