...
6. **QA et Autocritique stricte (Nouveau protocole septembre 2026) :** L'IA ne doit JAMAIS proposer de code non testé à Tobie en espérant qu'il trouve les bugs de logique. L'IA fait son propre QA : anticiper les impacts client/serveur (ex: bloquer une action sur serveur implique obligatoirement d'adapter le client pour éviter les erreurs "Permission Denied"), vérifier la compilation et la logique locale (`flutter analyze`, `firebase dry-run`) AVANT de soumettre le résultat comme étant prêt.
...
**Statut au 31/08/2026 : DÉPLOYÉ EN PRODUCTION** (version 1.18.0+49) — règles Firestore, index, toutes les fonctions Cloud (dont la nouvelle `gerer_participant_dossier`), l'app Android et web. Rituel complet exécuté et vérifié en ligne (`curl`, pas juste le message de succès des commandes) — voir `PROGRESS.md`, entrée "PREMIER DÉPLOIEMENT RÉEL" du 31/08/2026 pour le détail (y compris un vrai incident de mémoire pendant le build APK, corrigé). **Mais rien n'a encore tourné en conditions réelles avec de vrais comptes** — voir l'incertitude ci-dessous, toujours non vérifiée, désormais la priorité n°1.
...
**⚠️ PASSATION URGENTE (31/08/2026, fin de journée)** : Tobie a testé la Phase 1 en conditions réelles (deux comptes) le 31/08 et a remonté 9 problèmes concrets (voir `PROGRESS.md`, entrée "Test réel par Tobie"). La session suivante a conçu ET IMPLÉMENTÉ une **"Phase 2"** complète en réponse (plan validé en Plan Mode, écrit dans `/home/tobie/.claude/plans/ethereal-orbiting-crane.md` — lire ce fichier en premier si tu reprends ce chantier, il documente la conception complète). **Tout le code est écrit et vérifié localement (flutter analyze/test propres, dry-run des règles/fonctions OK, import Python OK) mais RIEN N'EST DÉPLOYÉ ET AUCUN RED TEAM COMPLET N'A ÉTÉ FAIT** — la session s'est arrêtée net (limite d'usage Claude Code atteinte) avant de pouvoir terminer la revue multi-agents lancée en fin de session (la plupart des 8 agents ont échoué pour la même raison — limite de session — un seul, "Angle E: simplification", a fini et a remonté 6 pistes de simplification mineures, aucune non plus vérifiée/appliquée). **Prochaine IA : commence par un red team complet de tout le diff avant même de penser à déployer.**

Ce que la Phase 2 ajoute (voir `PROGRESS.md` pour le détail fichier par fichier) :
- `permissionsAdministrateur` : le créateur choisit, administrateur par administrateur, s'il a le droit de gérer les participants (`gererParticipants`), supprimer le dossier (`supprimerDossier`), et/ou modifier/supprimer librement le contenu des AUTRES sans validation (`modererContenu`). Défaut désactivé pour tout nouvel administrateur ; absence totale du champ sur un dossier = comportement historique complet préservé (rétrocompatibilité).
- `propositionEnAttente` : workflow d'approbation sur tâches/journal — sans `modererContenu`, un administrateur est traité comme un simple contributeur : il propose une modification/un marquage fait/une suppression au lieu de l'appliquer directement, l'auteur ou un gestionnaire approuve ou rejette.
- 6 nouveaux types de notifications instantanées (ajout/retrait/rôle changé/action d'un administrateur/proposition créée/résolue) — dont la toute première fonction Cloud déclenchée par Firestore (`firestore_fn.on_document_written`) de ce projet.
- Attribution visible (qui a écrit quoi), "À traiter" en deux onglets (mes dossiers / dossiers partagés).
- **Deux trous trouvés en COURS D'IMPLÉMENTATION, pas anticipés par le plan initial** (transparence importante pour la suite) :
  1. Le plan initial gardait "créateur/administrateur" comme classe unique à accès libre — ne corrigeait AUCUNE des remarques de Tobie sur les administrateurs abusant de leurs droits. Reposé à Tobie en question directe, tranché : la permission `modererContenu` a été ajoutée en conséquence.
  2. La restriction "modifier nom/date du dossier réservé créateur/administrateur" avait été confirmée par Tobie pendant le planning mais jamais réellement codée dans `firestore.rules` avant cette session — corrigée, plus un conflit annexe découvert et corrigé (la cascade de `modifierDossier` vers les tâches, en écriture CLIENTE, entrait en conflit avec la nouvelle règle d'approbation — troisième chemin de règle dédié ajouté).

**Décisions de roadmap encore ouvertes** (voir `Notes/2026-08-30-redteam-code-collaboration.md`, section "Décisions de roadmap") :
- Groupes multi-dossiers : une future notion de "groupe"/"communauté" regroupant plusieurs dossiers liés pour une équipe qui travaille dessus ensemble. Principe d'accès déjà tranché par Tobie (raisonnement présenté et validé, voir règle 2 en section 1) : le groupe reste **organisationnel seulement**, jamais d'accès automatique aux dossiers qu'il contient. Rien conçu en détail au-delà de ce principe.
...
- **PRIORITÉ N°1 — Phase 2 (voir section 8) : red team complet AVANT tout déploiement.** Code écrit et vérifié localement, mais AUCUNE revue red team n'a abouti (la revue multi-agents lancée en fin de session a échoué sur limite d'usage) et RIEN n'est déployé. Ne pas déployer avant une vraie revue — c'est un chantier de permissions/sécurité, pas un chantier cosmétique. Une fois la revue faite et les correctifs appliqués : build + déploiement groupé (règles + fonctions + app, voir section 9), PUIS un nouveau test en conditions réelles avec Tobie et un deuxième compte (comme pour la Phase 1) avant de considérer le chantier terminé.
- La Phase 1 (accès de base par dossier) reste, elle, déjà déployée et testée en conditions réelles avec succès le 31/08/2026 (voir section 8) — ne pas reconfondre les deux statuts.
- Concevoir en détail les groupes multi-dossiers (voir section 8) — décision de principe prise, rien construit.
